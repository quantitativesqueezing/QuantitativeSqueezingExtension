...
function ensureTooltip() {
  ...
  let el = null;
  if (window.QSETooltipTemplate && typeof window.QSETooltipTemplate.createTooltipElement === 'function') {
    el = window.QSETooltipTemplate.createTooltipElement();
  }
  if (!el) {
...
function showLoading(symbol, referenceRect) {
  ...
  safeSetText('.shi-regsho-threshold', '—');
...
function showData(symbol, referenceRect, data) {
  ...
  const fieldsToCheck = [
    ...
    'optionsTradingEnabled', 'regShoThreshold',
  ...
  safeSetText('.shi-regsho-threshold', formatYesNoUnknown(data?.regShoThreshold));
